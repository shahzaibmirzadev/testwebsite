-- Monotonic "lifetime roles" counter (homepage). Safe to re-run (idempotent).
-- Same logic as supabase/migrations/20260415120000_site_metrics_lifetime_roles.sql
-- Increments on each new job row INSERT only.
...
-- Do not create an AFTER INSERT trigger here: daily-sync and pipeline call
-- increment_lifetime_roles_by(delta) so the counter stays correct without double-counting.
-- Migration 20260416120000_lifetime_roles_rpc_no_duplicate_trigger.sql drops any legacy trigger.
drop trigger if exists jobs_increment_lifetime_roles on public.jobs;

comment on table public.site_metrics is
  'Site-wide counters; lifetime_roles is updated by increment_lifetime_roles_by from job sync code.';
